...
import com.example.opengl_example.FileUtils.readTextFromRaw
import android.opengl.GLES20
import android.opengl.GLU
import android.util.Log
import com.example.opengl_example.ShaderUtils
...
object ShaderUtils {
    ...
    fun checkGlError(op: String) {
        ...
        if (GLES20.glGetError().also { error = it } != GLES20.GL_NO_ERROR) {
            Log.e(op, ": glError $error :${GLU.gluErrorString(error)}")
            throw java.lang.RuntimeException("$op: glError $error - ${GLU.gluErrorString(error)}")
        }
    }
}